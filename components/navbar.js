function top() {
    return `<div id="top-container" class="top">
                <div><p>Brand Waali Quality. Bazaar waali Deal!</p></div>
                <div>
                    <ul>
                        <li id="our-blog">Our Blog</li>
                        <li id="help-center">Help Center</li>
                        <li id="snap-deal">Sell On Snapdeal</li>
                        <li id="down-app">
                            <i class="fa-solid fa-mobile-screen-button"></i
                            ><span>Download App</span>
                        </li>
                        <li id="tourClick">
                            <i class="fa-solid fa-question"></i
                            ><span>Tour</span>
                        </li>
                    </ul>
                </div>
            </div>`;
}

function navbar() {
    return `<div class="navbar">
            <div class="nav">
                <div class="left">
                    <a href="/index.html"
                        ><img
                            src="./components/logo/snapdeal-logo.png"
                            alt="Snapdeal logo"
                    /></a>
                </div>
                <div class="middle" id="input-search">
                    <input
                        type="text"
                        name=""
                        id="search"
                        class="search-bar"
                        placeholder="Search products & brands"
                        autocomplete="off"
                    />
                    <button>
                        <i class="fa-solid fa-magnifying-glass"></i
                        ><span>Search</span>
                    </button>
                </div>
                <div class="right">
                    <div id="cart" class="cart brdr-left" onclick="window.location.href='./cart/cart.html'">
                    <p>Cart</p>
                    <i class="fa-solid fa-cart-shopping"></i>
                    </div>
                    <div class="pop-down brdr-left">
                        <div id="user" class="user">
                            <p>Sign In</p>
                            <i class="fa-regular fa-user"></i>
                        </div>
                    </div>
                    <div id="ham" class="ham">
                        <div class="your-account a">
                            <i class="fa-regular fa-user"></i>
                            <p>Your Account</p>
                        </div>
                        <div class="your-order a">
                            <i class="fa-solid fa-cube"></i>
                            <p>Your Orders</p>
                        </div>
                        <hr />
                        <div class="reg-log">
                            <p style="color: #b4b4b4;">If you are a new user</p>
                            <p onclick="window.location.href='./signup_login/login.html'">Register</p>
                            <button onclick="window.location.href='./signup_login/login.html'">LOGIN</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>`;
}

function footer() {
    return `<div class="footer">
            <div class="foot">
                <div class="informa">
                    <div class="secure-payments">
                        <img src="./components/logo/secure-payment.png" alt="" />
                        <p>100% secure payments</p>
                        <p>
                            Moving your card details to a much more secured
                            place
                        </p>
                    </div>
                    <div class="trustpay">
                        <img src="./components/logo/insurance.png" alt="" />
                        <p>Trustpay</p>
                        <p>100% Payment Protection. Easy Return Policy</p>
                    </div>
                    <div class="help-center">
                        <img src="./components/logo/help-desk.png" alt="" />
                        <p>help center</p>
                        <p>
                            Got a question? Look no further. Browse our FAQs or
                            submit your query here.
                        </p>
                    </div>
                    <div class="shop-on-go">
                        <img src="./components/logo/mobile-phone.png" alt="" />
                        <p>shop on the go</p>
                        <p>
                            Download the app and get exciting app only offers at
                            your fingertips
                        </p>
                    </div>
                </div>
            </div>

            <div class="footer-table">
                <ul>
                    <li>policy info</li>
                    <li><span>Privacy Policy</span></li>
                    <li><span>Terms of Sale</span></li>
                    <li><span>Terms of Use</span></li>
                    <li><span>Report Abuse & Takedown Policy</span></li>
                    <li><span>FAQ</span></li>
                </ul>
                <ul>
                    <li>Company</li>
                    <li><span>Import@snapdeal</span></li>
                    <li><span>Careers</span></li>
                    <li><span>Blog</span></li>
                    <li><span>Sitemap</span></li>
                    <li><span>Contact Us</span></li>
                </ul>
                <ul>
                    <li>snapdeal business</li>
                    <li><span>Shopping App</span></li>
                    <li><span>Sell on Snapdeal</span></li>
                    <li><span>Media Enquiries</span></li>
                </ul>
                <ul>
                    <li>popular links</li>
                    <li><span>Lehenga</span></li>
                    <li><span>Headphones</span></li>
                    <li><span>Kid's Clothing</span></li>
                    <li><span>Sarees</span></li>
                    <li><span>Shoes</span></li>
                    <li><span>Winter Wear</span></li>
                    <li><span>Shirt For Men</span></li>
                </ul>
                <ul class="last">
                    <li>Subscribe</li>
                    <li>
                        <input type="text" placeholder="Your email address" />
                        <button>subscribe</button>
                    </li>
                    <li>
                        Register now to get updates on promotions and coupons. <br>
                        <span
                            ><a class="on-download" href="#"
                                >Or Download App</a
                            ></span
                        >
                    </li>
                </ul>
            </div>
            <hr />
            <div class="payment-connect">
                <div class="payment-cards-details p-c">
                    <p>PAYMENT</p>
                    <ul>
                        <li>
                            <img
                                src="./components/logo/credit-card.png"
                                alt=""
                            />
                        </li>
                        <li>
                            <img
                                src="./components/logo/credit-cards.png"
                                alt=""
                            />
                        </li>
                        <li>
                            <img
                                src="./components/logo/credit-card (1).png"
                                alt=""
                            />
                        </li>
                        <li>
                            <img
                                src="./components/logo/credit-card (2).png"
                                alt=""
                            />
                        </li>
                        <li>
                            <img
                                src="./components/logo/cash-on-delivery.png"
                                alt=""
                            />
                        </li>
                        <li>
                            <img src="./components/logo/banking.png" alt="" />
                        </li>
                    </ul>
                </div>
                <div class="connect p-c">
                    <p>Connect</p>
                    <ul>
                        <li>
                            <img
                                src="https://img.icons8.com/fluency/48/000000/facebook-new.png"
                            />
                        </li>
                        <li>
                            <img
                                src="https://img.icons8.com/fluency/48/000000/twitter.png"
                            />
                        </li>
                        <li>
                            <img
                                src="https://img.icons8.com/fluency/48/000000/instagram-new.png"
                            />
                        </li>
                        <li>
                            <img
                                src="https://img.icons8.com/fluency/48/000000/linkedin-circled.png"
                            />
                        </li>
                        <li>
                            <img
                                src="https://img.icons8.com/fluency/48/000000/youtube-play.png"
                            />
                        </li>
                    </ul>
                </div>
            </div>
            <hr />
            <div class="copyright">
                <p>
                    Copyright &#169; 2021, Snapdeal Limited. All Rights Reserved
                </p>
                <span
                    >Made for Bharat<img
                        class="dancing-heart"
                        src="./components/logo/icons8-heart.gif"
                /></span>
            </div>
        </div>`;
}




export { top, navbar, footer };
